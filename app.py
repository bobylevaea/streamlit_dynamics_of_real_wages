...
if __name__ == '__main__':
    st.sidebar.title("Анализ динамики номинальных и реальных заработных плат в Российской Федерации в период с 2000 по 2023 годы по отраслям")
...
''')
    ...
    st.sidebar.write('**Параметры датасета:**')
    for key, value in parameters.items():
      st.sidebar.write(f'- **{key}**: {value}')
    
     
    st.write('#### Номинальные и реальные заработные платы, годовая инфляция, уровень безработицы, номинальный ВВП на душу населения Российской Федерации в долларах США в период с 2000 по 2023 годы по отраслям')
    ...
    st.write('#### Динамика номинальных и реальных заработных плат в период с 2000 по 2023 годы по отраслям')
  

    # Значения для кнопок выбора
    options = ['Строительство', 'Образование', 'Здравоохранение и социальные услуги']

    # Задаем значение по умолчанию
    default_option = options[0]  # например, выбираем первый вариант по умолчанию

    # Создаем кнопки выбора
    selected_option = st.radio("#### Выберите отрасль:", options, index=options.index(default_option))

    # Отображаем графики в зависимости от выбранной отрасли
    if selected_option == 'Строительство':
       st.write('#### Строительство')
       fig2, ax2 = create_plot_2(data)
       st.pyplot(fig2)
    elif selected_option == 'Образование':
       st.write('#### Образование') 
       fig3, ax3 = create_plot_3(data)
       st.pyplot(fig3)
    elif selected_option == 'Здравоохранение и социальные услуги':
       st.write('#### Здравоохранение и социальные услуги')
